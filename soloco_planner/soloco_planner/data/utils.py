...
def prepare_states(x, hist, num_agent, full_return= False, bc= False, device='cuda'):
    # x = torch.Tensor(x)
    ...
    with torch.no_grad():
        ...
        if not bc:
            ...
            current_szene = obs_traj_pos[:, -1].sum(dim=-1)
            inx_peds_in_seq = torch.where(current_szene != 0, True, False)
            ...
            obs_traj_pos = x[:, :n].reshape(batch, hist + 2, num_agent + 1, 2)
            current_szene = obs_traj_pos[:, -2].sum(dim=-1)
            inx_peds_in_seq = torch.where(current_szene != 0, True, False)

            obs_traj_pos = obs_traj_pos.permute(1, 0, 2, 3).reshape(hist + 2, -1, 2)
            cum_start_idx = torch.cumsum(torch.cat([zero_start, inx_peds_in_seq.sum(dim=-1)]),
                                         dim=-1)  # ToDo:remove .to

        inx_peds_in_seq = inx_peds_in_seq.view(-1)
    ...
    if bc:
        neigh_next = obs_traj_pos[-1]
        obs_traj_pos = obs_traj_pos[:-1]
        data = [obs_traj_pos, traj_rel, neigh_indx, cum_start_idx[:-1], r_goal, robot_state, neigh_next]
    else:
...
def fast_coll_counter(scene, robot_id, coll_dist= 0.2): # todo: check only robot coll!!!!!
    ...
    return filter_zeros.sum()

def robot_dist_for_Eval(scene, robot_id): # todo: check only robot coll!!!!!
    dist_mat = torch.cdist(scene, scene, p=2.0, compute_mode='donot_use_mm_for_euclid_dist')
...
def evaluate(policy, env, agent_hist, human_future, goal_thresh= 0.2, print_all = True, render=False, epoch=1):
    epoch_results = []
    for e in range(epoch):
        print('Start epoch ' +str(e + 1) + '/' + str(epoch))
        actions, scenes, robot_ids, \
        goals_dis, goals, success, path_len_h, \
        path_len_r, timeout, inference_t = env.get_dataset(agent='robot', policy=policy, train=False, render=render)
        min_distance_s_r, less_2_s_r, less_3_s_r, densities_r = [], [], [], []
        min_distance_s_h, less_2_s_h, less_3_s_h, densities_h = [], [], [], []
        for i, scene in enumerate(scenes):
            scene_with_robot = scene.copy()
