...
import numpy as np
from nav2_soloco_controller.models.utils import Pooling_net, Hist_Encoder

from  nav2_soloco_controller.data.utils import dynamic_window, actionXYtoROT, GaußNLL
...
class TrajectoryGeneratorAR_goal(nn.Module):
    ...
    def forward(self, traj_rel, obs_traj_pos,
                nei_index, robot_state, sample_goal, z=None, robotID=None, optimize_latent_space=False, mean_pred=True):
        ...
        for i in range(self.pred_len):
            ...
            test_corr_index = (test_corr.transpose(1, 2) - test_corr) #  + 10
            ...
            if self.robot_params_dict['use_robot_model']:
                ...
                output_pred = torch.clamp(output_pred,
                                                  min=self.robot_params_dict["min_speed"] * self.dt,
