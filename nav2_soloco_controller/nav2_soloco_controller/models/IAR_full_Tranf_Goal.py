...
from nav2_soloco_controller.models.utils import Hist_Encoder
import numpy as np
class Decoder_TF(nn.Module):
    def __init__(self, device, input_dim=16):
        ...
        self.input_fc = nn.Linear(4, self.d_model)
...
class TrajectoryGeneratorGoalIAR(nn.Module):
    ...
    def forward(self, traj_rel, obs_traj_pos,
        ...
        if calc_new:
            ...
            rel_goal = (rel_goal[:num_h]).unsqueeze(dim=0).repeat(self.predictions_steps, 1, 1)
            enc_hist = self.hist_encoder(traj_rel[: self.obs_len, :num_h])
            # noise_sampled[:, robotID] = 0.
            context = torch.cat([torch.zeros_like(noise_sampled[:, :num_h]), rel_goal], dim=-1)
