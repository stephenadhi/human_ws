...
SubGoalPublisher::SubGoalPublisher()
    ...
    this->declare_parameter<bool>("use_velocity_scaled_lookahead_dist", true);
    ...
    this->get_parameter("use_velocity_scaled_lookahead_dist", use_velocity_scaled_lookahead_dist_);
...
void SubGoalPublisher::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg){
    ...
    if (use_velocity_scaled_lookahead_dist_) {
        lookahead_dist_ = getLookAheadDistance(msg->twist.twist);
    }
}
...
void SubGoalPublisher::globalPlanCallback(const nav_msgs::msg::Path::SharedPtr msg){
    ...
    auto goal_pose_it = std::find_if(
        ...
        return std::hypot(ps.pose.position.x- robot_pose_.position.x, 
            ps.pose.position.y - robot_pose_.position.y) >= lookahead_dist_;
        }
    );
    ...
    subgoal_pose.header.frame_id = frame_id_;
