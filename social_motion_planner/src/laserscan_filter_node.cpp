...
#include <zed_interfaces/msg/bounding_boxes_3d.hpp>
...
public:
    LaserScanFilterNode()
        ...
        this->get_parameter("scan_topic", laserscan_topic);
...
private:
    ...
    // Store the 2D bounding box information for use in filtering the laserscan
        ...
        for (int i = 0; i < num_agents; i++) {
            const auto& corners = msg->objects[i].bounding_boxes_3d.corners;
            std::array<float, 8> bounding_box;
            bounding_box.assign(corners.begin(), corners.end());
