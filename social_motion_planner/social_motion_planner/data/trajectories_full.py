...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        for path in all_files:
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                ...
                for _, ped_id in enumerate(peds_in_curr_seq):
                    ...
                    # sdfg = curr_ped_seq[-1, 0]
                    # test = frames.index(curr_ped_seq[-1, 0])
        ...
        # maxx, minx = np.max(seq_list[:, 0, :]), np.min(seq_list[:, 0, :])
        # maxy, miny = np.max(seq_list[:, 1, :]), np.min(seq_list[:, 1, :])
...
class ObsDataset(Dataset):
    def __init__(self,dataset):
        ...
        self.actions = dataset.actions #.reshape([-1,11,2])
        self.obs = dataset.observations #.reshape([-1,11,1012])
